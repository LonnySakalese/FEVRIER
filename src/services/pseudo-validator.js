...
const DENY_LIST = [
    // Insultes FR
    'pute', 'putain', 'salope', 'connard', 'connasse', 'enculé', 'enculer', 'batard', 'bâtard',
    'fdp', 'ntm', 'nique', 'niquer', 'merde', 'bordel', 'pd', 'tapette', 'gouine',
    'triso', 'mongol', 'débile', 'attardé', 'retardé', 'abruti', 'crétin', 'idiot',
    'fils de', 'ta mere', 'ta mère', 'tg', 'ftg', 'stfu',
    // Insultes EN
    'fuck', 'shit', 'bitch', 'asshole', 'dick', 'pussy', 'cunt', 'whore', 'slut',
    'bastard', 'retard', 'faggot', 'fag', 'nigger', 'nigga', 'negro',
    // Violence
    'tueur', 'killer', 'murder', 'tuer', 'mourir', 'suicide', 'suicid',
    'vengeance', 'revenge', 'destroy', 'détruire', 'destruire',
    'lame', 'blade', 'gun', 'fusil', 'bombe', 'bomb', 'terroris',
    'viol', 'rape', 'blood', 'sang', 'death', 'mort', 'die', 'crever',
    // Extrémisme
    'nazi', 'hitler', 'fascis', 'racis', 'jihad', 'isis', 'heil',
    'supremac', 'kkk', 'skinhead',
    // Sexuel
    'porn', 'xxx', 'sex', 'bite', 'couille', 'penis', 'vagin',
    'orgasm', 'fetish', 'hentai', 'milf',
    // Négatif profond
    'echectotal', 'failur', 'worthless', 'useless', 'inutile',
    'jedéteste', 'jehais', 'ihate', 'nolife', 'loser',
    // Usurpation
    'admin', 'moderateur', 'moderator', 'modo', 'staff', 'support', 'system', 'bot',
];
...
export function validatePseudo(pseudo) {
    ...
    if (!/^[a-zA-Z0-9À-ÿ _\-\.]+$/.test(trimmed)) {
        return {
            is_valid: false,
            reason: 'Caractères spéciaux non autorisés. Utilise des lettres, chiffres, espaces ou tirets',
    ...
    for (const word of DENY_LIST) {
        if (lower.includes(word.replace(/\s/g, ''))) {
            return {
                is_valid: false,
                reason: 'Ce pseudo contient un terme interdit',
                suggested_alternatives: generateAlternatives()
            };
        }
    }
    ...
    for (const pattern of negativePatterns) {
        if (pattern.test(trimmed)) {
            return {
                is_valid: false,
                reason: 'Ce pseudo a une connotation trop négative. Choisis quelque chose de positif !',
                suggested_alternatives: generateAlternatives()
            };
        }
    }
...
/**
 * Génère des suggestions de pseudos aléatoires
 */
function generateAlternatives() {
    const adjectives = [
        'Brave', 'Fort', 'Rapide', 'Grand', 'Sage', 'Noble', 'Fier',
        'Vaillant', 'Agile', 'Calme', 'Bold', 'Swift', 'Iron', 'Storm'
    ];
    const nouns = [
        'Guerrier', 'Phoenix', 'Lion', 'Titan', 'Dragon', 'Hawk',
        'Wolf', 'Eagle', 'Tiger', 'Falcon', 'Champion', 'Warrior'
    ];
