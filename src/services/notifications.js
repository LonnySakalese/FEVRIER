...
export async function initNotifications() {
    ...
    try {
        ...
        if (firebase.messaging) {
            ...
            try {
                const configDoc = await firebase.firestore().collection('config').doc('fcm').get();
                if (configDoc.exists) {
            ...
            if (vapidKey) {
                ...
                console.warn('No VAPID key configured — FCM disabled, local notifications only');
            }
        }
...
export async function renderNotifSettings(containerId) {
    ...
    // Toggle handlers
    const toggles = ['notifDailyReminder', 'notifNewMessage', 'notifMemberJoined'];
    toggles.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.addEventListener('change', async () => {
                const newPrefs = {
                    dailyReminder: document.getElementById('notifDailyReminder')?.checked ?? true,
                    reminderHour: parseInt(document.getElementById('notifReminderHour')?.value) || 20,
                    newMessage: document.getElementById('notifNewMessage')?.checked ?? true,
                    memberJoined: document.getElementById('notifMemberJoined')?.checked ?? true,
                };
                await saveNotifPrefs(newPrefs);

                // Show/hide reminder hour
                const hourRow = document.getElementById('reminderHourRow');
                if (hourRow) hourRow.style.display = newPrefs.dailyReminder ? '' : 'none';
            });
        }
    });
