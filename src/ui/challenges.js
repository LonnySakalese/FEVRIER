    ...
    if (habitsContainer) {
        try {
            ...
            if (habitNames.length === 0) {
                habitsContainer.innerHTML = '<div style="color: var(--accent-dim); text-align: center; padding: 10px;">Aucune habitude dans ce groupe</div>';
            } else {
                habitsContainer.innerHTML = habitNames.map(name => `
            ...
            habitsContainer.innerHTML = '<div style="color: var(--accent-dim);">Erreur chargement habitudes</div>';
        }
    }
...
export async function createChallenge() {
    ...
    // Get selected habits
    const checkboxes = document.querySelectorAll('#challengeHabitsList input[type="checkbox"]:checked');
    ...
    try {
        ...
        // Create challenge
        const challengeRef = await db.collection('groups').doc(groupId).collection('challenges').add({
            name,
            description,
            creatorId: userId,
        ...
        await db.collection('groups').doc(groupId).collection('challenges').doc(challengeRef.id)
        ...
        // Auto message
        await sendAutoMessage(groupId, `⚔️ Nouveau challenge "${name}" lancé par ${userData.pseudo || 'Anonyme'} ! Durée : ${duration} jours. Rejoignez le combat ! 💪`);
...
export async function renderChallenges(groupId) {
    ...
    container.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--accent-dim);">⏳ Chargement...</div>';

    try {
        const snap = await db.collection('groups').doc(groupId).collection('challenges')
            ...
            container.innerHTML = `
                <div class="challenge-empty">
                    ...
                    <div style="font-size: 0.8rem; margin-top: 8px; color: var(--accent-dim);">
                        Lance le premier challenge du groupe !
                    </div>
                </div>`;
            return;
        }
        ...
        for (const doc of snap.docs) {
            ...
            const progressPct = c.status === 'active' ? Math.max(0, Math.round(((totalDays - daysLeft) / totalDays) * 100)) : 100;
        ...
        container.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--accent-dim);">❌ Erreur de chargement</div>';
    }
}
...
export async function joinChallenge(groupId, challengeId) {
    ...
    try {
        ...
        await db.collection('groups').doc(groupId).collection('challenges').doc(challengeId)
            .collection('participants').doc(userId).set({
                pseudo: userData.pseudo || 'Anonyme',
                avatar: userData.avatar || '👤',
                joinedAt: firebase.firestore.FieldValue.serverTimestamp()
        ...
        await db.collection('groups').doc(groupId).collection('challenges').doc(challengeId).update({
...
export async function openChallengeDetail(groupId, challengeId) {
    ...
    container.innerHTML = '<div style="text-align:center; padding: 30px; color: var(--accent-dim);">⏳ Chargement du classement...</div>';

    try {
        const cDoc = await db.collection('groups').doc(groupId).collection('challenges').doc(challengeId).get();
        if (!cDoc.exists) {
            container.innerHTML = '<div style="text-align:center; padding: 20px;">Challenge introuvable</div>';
            return;
        }
        const c = cDoc.data();
        const userId = appState.currentUser?.uid;
        ...
        const daysLeft = isActive ? getDaysRemaining(c.endDate) : 0;
        const totalDays = c.duration || 7;
        const progressPct = isActive ? Math.max(0, Math.round(((totalDays - daysLeft) / totalDays) * 100)) : 100;

        // Get participants
        const participantsSnap = await db.collection('groups').doc(groupId)
            .collection('challenges').doc(challengeId)
            .collection('participants').get();
        ...
        const startDate = c.startDate.toDate ? c.startDate.toDate() : new Date(c.startDate);
        ...
        for (const pDoc of participantsSnap.docs) {
            ...
            try {
                // Get member's habits
                const memberHabitsSnap = await db.collection('users').doc(memberId).collection('habits').get();
                const memberHabits = {};
                memberHabitsSnap.docs.forEach(h => {
                    memberHabits[h.id] = h.data().name;
                });

                const challengeHabitNamesLower = (c.habitNames || []).map(n => n.trim().toLowerCase());
                const relevantHabitIds = Object.entries(memberHabits)
                    .filter(([, name]) => challengeHabitNamesLower.includes(name.trim().toLowerCase()))
                    .map(([id]) => id);
                ...
                if (relevantHabitIds.length > 0) {
                    ...
                    while (curDate <= effectiveEnd) {
                        ...
                        const dayCompleted = relevantHabitIds.filter(id => completedIds.includes(id)).length;
                    ...
                    score = totalDaysChecked > 0 ? Math.round((totalCompleted / totalDaysChecked) * 100) : 0;
                }
            } catch (e) {
        ...
        container.innerHTML = `
            <div class="challenge-detail">
                ...
                <div class="challenge-leaderboard">
                    ${participants.length === 0 ? '<div style="text-align:center; padding: 15px; color: var(--accent-dim);">Aucun participant</div>' :
...
async function finishChallenge(groupId, challengeId, challengeData) {
    try {
        ...
        const medals = ['🥇', '🥈', '🥉'];
        const podium = top3.map((r, i) => `${medals[i]} ${r.pseudo} (${r.score}%)`).join('\n');
        await sendAutoMessage(groupId, `✅ Challenge "${c.name}" terminé !\n\n🏆 Classement final :\n${podium}\n\nBravo à tous les participants ! 🎉`);
