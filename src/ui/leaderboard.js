...
export async function renderLeaderboard(groupId) {
    ...
    try {
        ...
        const groupHabitNamesLower = (groupData.habitNames || []).map(n => n.trim().toLowerCase());
        ...
        for (const mDoc of membersSnap.docs) {
            ...
            try {
                ...
                for (const dayKey of last7Days) {
                    ...
                    const dayScore = Math.round((completed / relevantHabitIds.length) * 100);
        ...
        html += `<div class="leaderboard-footer">Basé sur les 7 derniers jours</div>`;
